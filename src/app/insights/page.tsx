  ...
  return (
    <>
      ...
      <main className="flex-1 bg-sand-50 py-10">
        <Container>
          <h1 className="text-2xl font-semibold text-ink-950">Market Insights</h1>
          <p className="mt-1 text-sm text-sand-600">Price and rental trends across Dubai&apos;s communities.</p>

          {withMetrics.length === 0 ? (
            <div className="mt-16 rounded-2xl border border-dashed border-sand-300 bg-white py-16 text-center">
              <p className="text-sand-600">No market metrics recorded yet.</p>
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-sand-200 bg-sand-50 text-left text-sand-600">
                    ...
                    <th className="px-4 py-3 font-medium">AED/sqft</th>
                    <th className="px-4 py-3 font-medium">Avg. annual rent</th>
                    ...
                    return (
                      <tr key={area.slug} className="border-b border-sand-100 last:border-0">
                        <td className="px-4 py-3">
                          ...
                          {m.isDemoData && <DemoDataBadge className="ml-2" />}
                        </td>
