  ...
  return (
    <>
      ...
      <main className="flex-1 bg-sand-50 py-10">
        <Container>
          <h1 className="text-2xl font-semibold text-ink-950">{dict.areas.pageTitle}</h1>
          <p className="mt-1 text-sm text-sand-600">{dict.areas.pageSubtitle}</p>
              ...
              <Link key={area.slug} href={`/areas/${area.slug}`}>
                <Card className="p-4 transition-shadow hover:shadow-md">
                  <div className="flex items-start justify-between">
                    ...
                    {area.latestMetric?.isDemoData && <DemoDataBadge label={dict.common.demoData} />}
                  </div>
                  <p className="mt-3 text-xs text-sand-600">
                    {area.listingCount.toLocaleString()} {dict.areas.listingsLabel}
                  </p>
                  {area.latestMetric?.avgPricePerSqftAed && (
                    <p className="mt-1 text-sm font-semibold text-ink-950">
                      {formatAed(Number(area.latestMetric.avgPricePerSqftAed))}{" "}
                      <span className="font-normal text-sand-600">/ {dict.common.sqft}</span>
                    </p>
                  )}
                </Card>
              </Link>
